...
var log = clog.NewWithPlugin("rest")
...
func (r Rest) ServeDNS(ctx context.Context, w dns.ResponseWriter, m *dns.Msg) (int, error) {
	state := request.Request{W: w, Req: m}
	qname := state.Name()
	
	log.Debugf("Received query for %s (type %d)", qname, state.QType())
	log.Debugf("Constructing API URL: %s with qname: %s", r.URL, qname)

	// Build REST API URL with query name
	url := fmt.Sprintf("%s%s", r.URL, qname)
